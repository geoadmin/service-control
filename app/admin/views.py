...
def oauth2_proxy_base_url(request: HttpRequest) -> str:
...
def custom_admin_login(request: HttpRequest) -> HttpResponse:
    ...
    oauth2_proxy_login_url = \
        f"{oauth2_proxy_base_url(request)}oauth2/start?" + \
...
def custom_admin_logout(request: HttpRequest) -> HttpResponse:
    ...
    oauth_proxy_logout_url = f'{oauth2_proxy_base_url(request)}oauth2/sign_out?' + \
