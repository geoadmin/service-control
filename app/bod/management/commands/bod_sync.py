...
class Handler(CommandHandler):
    ...
    def import_providers(self) -> None:
        ...
        for organization in BodContactOrganisation.objects.all():
            if (
                not organization.attribution
                or len(organization.attribution.split(".")) != 2
            ):
        ...
        orphans = Provider.objects.filter(_legacy_id__isnull=False).exclude(
            _legacy_id__in=processed
        )
    ...
    def import_attribution(self) -> None:
        ...
        for organization in BodContactOrganisation.objects.all():
            ...
            if organization.attribution:
                provider_of_attribution = ".".join(
                    organization.attribution.split(".", 2)[:2]
                )
                provider = Provider.objects.filter(
                    provider_id=provider_of_attribution
                ).first()
            if not provider:
        ...
        orphans = Attribution.objects.filter(_legacy_id__isnull=False).exclude(
            _legacy_id__in=processed
        )
        _, removed = orphans.delete()
    ...
    def update_attribution(
        ...
        any_changed = False
        translation = BodTranslations.objects.filter(
            msg_id=organization.attribution
        ).first()
        for attribution_attribute, translation_attribute in (
            ...
            changed = self.update_model(
                ...
                getattr(translation, translation_attribute, "")
                or organization.attribution
                or "undefined",
                is_new_model,
            )
            any_changed = any_changed or changed
    ...
    def import_datasets(self) -> None:
        ...
        for bod_dataset in BodDataset.objects.filter(staging="prod").all():
            ...
            bod_meta = BodGeocatPublish.objects.filter(
                fk_id_dataset=bod_dataset.id_dataset
            ).first()
            if not bod_meta:
        ...
        orphans = Dataset.objects.filter(_legacy_id__isnull=False).exclude(
            _legacy_id__in=processed
        )
        _, removed = orphans.delete()
        for model, count in removed.items():
    ...
    def run(self) -> None:
        ...
        with transaction.atomic():
            ...
            if (
                not self.clear
                and not self.include_providers
                and not self.include_attributions
                and not self.include_datasets
            ):
                printed = True
