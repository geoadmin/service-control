...
class Handler(CommandHandler):
    ...
    def clear_users(self) -> None:
        ...
        for user in self.client.list_users():
            ...
            if not self.dry_run:
                ...
                if not deleted:
                    self.print_error(
                        "Could not delete %s, might not exist or might be unmanged",
                        user_id,
                    )
    ...
    def add_user(self, user: User) -> None:
        ...
        if not self.dry_run:
            ...
            if not created:
                self.print_error(
                    "Could not create %s, might already exist as unmanaged user",
                    user.user_id,
                )
    ...
    def delete_user(self, user_id: str) -> None:
        ...
        if not self.dry_run:
            ...
            if not deleted:
                self.print_error(
                    "Could not delete %s, might not exist or might be unmanaged",
                    user_id,
                )
    ...
    def update_user(self, local_user: User, remote_user: "UserTypeTypeDef") -> None:
        ...
        changed = local_user.email != remote_attributes.get(
            "email"
        ) or local_user.username != remote_attributes.get("preferred_username")
        if changed:
    ...
    def run(self) -> None:
        ...
        if self.clear:
            self.print_warning(
                "This action will delete all managed users from cognito", level=0
            )
