...
def test_get_users_returns_users_ordered_by_id(
...
def test_post_users_creates_new_user_in_db_and_returns_it(
    ...
    response = client.post(
        "/api/v1/users", data=payload, content_type="application/json"
    )
...
@patch("access.models.Client")
def test_post_users_returns_503_if_cognito_down(
    cognito_client, user, django_user_factory, client
):
...
@patch("access.models.Client")
def test_post_user_returns_401_if_not_logged_in(
    cognito_client, user, django_user_factory, client
):
    cognito_client.return_value.create_user.return_value = True
...
@patch("access.models.Client")
def test_post_user_returns_403_if_no_permission(
    cognito_client, user, django_user_factory, client
):
    cognito_client.return_value.create_user.return_value = True
...
@patch("access.models.Client")
def test_delete_user_returns_404_if_nonexisting(
    cognito_client, user, django_user_factory, client
):
...
def test_delete_user_returns_401_if_not_logged_in(
    ...
    response = client.delete(
        "/api/v1/users/dude", data={}, content_type="application/json"
    )
...
def test_update_user_updates_existing_user_as_expected(
    ...
    response = client.put(
        "/api/v1/users/dude", data=payload, content_type="application/json"
    )
