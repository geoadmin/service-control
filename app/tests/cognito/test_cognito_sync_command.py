...
def fixture_user(provider):
    with patch('access.models.Client') as client:
        ...
        yield User.objects.create(
            username='1', first_name='1', last_name='1', email='1@example.org', provider=provider
        )
...
def test_command_adds(cognito_client, user):
    ...
    assert 'adding user 1' in out.getvalue()
    assert '1 user(s) added' in out.getvalue()
    assert call().create_user('1', '1@example.org') in cognito_client.mock_calls


@patch('cognito.management.commands.cognito_sync.Client')
def test_command_deletes(cognito_client, db, cognito_user_response_factory):
    cognito_client.return_value.list_users.return_value = [
        cognito_user_response_factory('1', '1@example.org')
    ]

    out = StringIO()
    call_command('cognito_sync', verbosity=2, stdout=out)

    assert 'deleting user 1' in out.getvalue()
    assert '1 user(s) deleted' in out.getvalue()
    assert call().delete_user('1') in cognito_client.mock_calls


@patch('cognito.management.commands.cognito_sync.Client')
def test_command_updates(cognito_client, user, cognito_user_response_factory):
    cognito_client.return_value.list_users.return_value = [
        cognito_user_response_factory('1', '2@example.org')
    ]

    out = StringIO()
    call_command('cognito_sync', verbosity=2, stdout=out)

    assert 'updating user 1' in out.getvalue()
    assert '1 user(s) updated' in out.getvalue()
    assert call().update_user('1', '1@example.org') in cognito_client.mock_calls
...
def test_command_updates_disabled(cognito_client, user, cognito_user_response_factory):
    ...
    assert 'disabling user 1' in out.getvalue()
    assert '1 user(s) disabled' in out.getvalue()
    assert call().disable_user('1') in cognito_client.mock_calls


@patch('cognito.management.commands.cognito_sync.Client')
def test_command_updates_enabled(cognito_client, user, cognito_user_response_factory):
    cognito_client.return_value.list_users.return_value = [
        cognito_user_response_factory('1', '1@example.org', False)
    ]

    out = StringIO()
    call_command('cognito_sync', verbosity=2, stdout=out)

    assert 'enabling user 1' in out.getvalue()
    assert '1 user(s) enabled' in out.getvalue()
    assert call().enable_user('1') in cognito_client.mock_calls
...
@patch('cognito.management.commands.cognito_sync.Client')
def test_command_runs_dry(cognito_client, provider, user, cognito_user_response_factory):
    User.objects.create(
        username='2', first_name='2', last_name='2', email='2@example.org', provider=provider
    )

    cognito_client.return_value.list_users.return_value = [
        cognito_user_response_factory('1', '10@example.org'),
        cognito_user_response_factory('3', '3@example.org')
    ]

    out = StringIO()
    call_command('cognito_sync', dry_run=True, verbosity=2, stdout=out)

    assert 'adding user 2' in out.getvalue()
    assert 'deleting user 3' in out.getvalue()
    assert 'updating user 1' in out.getvalue()
