...
def test_list_users_returns_only_managed(boto3, cognito_user_response_factory):
    ...
    assert (
        call().list_users(UserPoolId=client.user_pool_id, Limit=60) in boto3.mock_calls
    )
...
def test_list_users_pagination(boto3, cognito_user_response_factory):
    ...
    assert [user["Username"] for user in users] == [
        f"2ihg2ox304po{x}" for x in range(1, 131)
    ]
    assert (
        call().list_users(UserPoolId=client.user_pool_id, Limit=60) in boto3.mock_calls
    )
...
@patch("cognito.utils.client.client")
def test_create_user_does_not_create_if_managed_exists(
    boto3, cognito_user_response_factory
):
    boto3.return_value.admin_get_user.return_value = cognito_user_response_factory(
        "2ihg2ox304po", "1234", managed=True, attributes_key="UserAttributes"
    )
...
@patch("cognito.utils.client.client")
def test_create_user_does_not_create_if_unmanaged_exists(
    boto3, cognito_user_response_factory
):
    boto3.return_value.admin_get_user.return_value = cognito_user_response_factory(
        "2ihg2ox304po", "1234", managed=False, attributes_key="UserAttributes"
...
def test_update_user_updates_managed(boto3, cognito_user_response_factory):
    ...
    assert (
        call().admin_reset_user_password(
...
def test_update_user_updates_partial_managed(boto3, cognito_user_response_factory):
    ...
    assert (
        call().admin_update_user_attributes(
            UserPoolId=client.user_pool_id,
            Username="2ihg2ox304po",
            UserAttributes=[{"Name": "preferred_username", "Value": "5678"}],
        )
        in boto3.mock_calls
    )
...
@patch("cognito.utils.client.client")
def test_update_user_does_not_update_unchanged_managed(
    boto3, cognito_user_response_factory
):
    boto3.return_value.admin_get_user.return_value = cognito_user_response_factory(
        "2ihg2ox304po", "1234", managed=True, attributes_key="UserAttributes"
    ...
    assert updated is True
    assert "().admin_update_user_attributes" not in [
        call[0] for call in boto3.mock_calls
    ]
    assert "().admin_reset_user_password" not in [call[0] for call in boto3.mock_calls]
