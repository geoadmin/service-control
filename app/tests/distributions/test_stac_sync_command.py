...
def test_command_imports(get, stac_client, provider, attribution):
    ...
    out = out.getvalue()
    assert (
        "Added package distribution 'ch.bafu.alpweiden-herdenschutzhunde' (managed)"
        in out
    )
    assert (
        "Added package distribution 'ch.bafu.hydrologie-hintergrundkarte' (unmanaged)"
        in out
    )
...
@patch("distributions.management.commands.stac_sync.Client")
@patch("distributions.management.commands.stac_sync.get")
def test_command_fails_to_import_if_dataset_is_missing(
    get, stac_client, provider, attribution
):
    stac_client.open.return_value.collection_search.return_value.collections.return_value = [
        Collection(
            id="ch.bafu.alpweiden-herdenschutzhunde",
...
@patch("distributions.management.commands.stac_sync.Client")
@patch("distributions.management.commands.stac_sync.get")
def test_command_fails_if_default_dataset_is_missing(
    get, stac_client, provider, attribution
):
    stac_client.open.return_value.collection_search.return_value.collections.return_value = [
        Collection(
...
def test_command_removes_orphans(get, stac_client, provider, attribution):
    ...
    get.return_value.text = (
        '<div id="data">remove_missing_dataset_html\nkeep_html</div>'
    )
...
@patch("distributions.management.commands.stac_sync.Client")
@patch("distributions.management.commands.stac_sync.get")
def test_command_removes_orphans_default_datset(
    get, stac_client, provider, attribution
):
    dataset = Dataset.objects.create(
        dataset_id="ch.bafu.alpweiden-herdenschutzhunde",
    ...
    out = StringIO()
    call_command(
        "stac_sync", verbosity=2, stdout=out, default_dataset=dataset.dataset_id
    )
    out = out.getvalue()
...
def test_command_clears(get, stac_client, provider, attribution):
    ...
    assert (
        PackageDistribution.objects.first().package_distribution_id
        == "ch.bafu.alpweiden-herdenschutzhunde.3"
    )
...
@patch("distributions.management.commands.stac_sync.Client")
@patch("distributions.management.commands.stac_sync.get")
def test_command_warns_about_multiple_providers(
    get, stac_client, provider, attribution
):
    Dataset.objects.create(
        dataset_id="ch.bafu.alpweiden-herdenschutzhunde",
    ...
    assert "1 package_distribution(s) added" in out
    assert (
        "Collection 'ch.bafu.alpweiden-herdenschutzhunde' has more than one provider"
        in out
    )
    assert PackageDistribution.objects.first()
...
@patch("distributions.management.commands.stac_sync.Client")
@patch("distributions.management.commands.stac_sync.get")
def test_command_does_not_warn_about_similar_provider(
    get, stac_client, provider, attribution
):
    Dataset.objects.create(
        dataset_id="ch.bafu.alpweiden-herdenschutzhunde",
